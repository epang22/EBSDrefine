function EBSDrefine_pseudo( data )
%EBSDREFINE_PSEUDO
...
% Only works for pseudosymmetric materials
...
% 
% data.h5output = '200220_EBSDrefine_test.h5';   % Name of .h5 output file (located in path)
% data.angoutput = '200220_EBSDrefine_test.ang';   % Name of .ang output file (located in path)
% 
...
%%% Compute pseudosymmetry rotation matrices
...
if reduceeuler==1
    ...
    for ii=1:N2
        ...
        for jj=1:size(eulerlist1_disorient,1)
            ...
            % don't include if it's within a pseudosym rotation of an
            % existing angle, because we're going to check them later anyways
            for kk=1:Npseudo
...
% add pseudosym angles to list of grid centers (stored separately)
eulerlist_pseudo = zeros(Neuler,3,Npseudo);
for ii=1:Neuler
    euler2 = eulerlist(ii,:);   % get euler angles for this iter
    for kk=1:Npseudo
        Reuler2 = eu2om(euler2*pi/180);     % orientation matrix of eulerlist angle
        R2p = Rrot{kk}*Reuler2;   % orientation matrix of pseudosym var of eulerlist angle
        euler2p = om2eu(R2p)*180/pi;   % euler angles of pseudosymmetry variant
        eulerlist_pseudo(ii,:,kk) = euler2p;
    end
end
...
% pseudosym vars
eulerlist_pseudo_grids = zeros(Neuler*gridsize,3,Npseudo);   % contains all refined grids about each angle in eulerlist
for ii=1:Neuler
    for kk=1:Npseudo
        euler1 = eulerlist_pseudo(ii,:,kk);
        [test_angles, test_angles_1Drodrigues] = createorigrid(delta_angles, N_angles, euler1);   % Create test grid of angles
        eulerlist_pseudo_grids(gridsize*(ii-1)+1:ii*gridsize,:,kk) = test_angles;
    end
end


%%% compile dictionary
eulerlist_grids_all = eulerlist_grids;  % original variant
% add on each pseudosym var
for kk=1:Npseudo
    eulerlist_grids_all = [eulerlist_grids_all; eulerlist_pseudo_grids(:,:,kk)];
end
...
%   dim1: euler angles (deg) then dp, dim2: map points, dim3: pseudosym vars
maxinterppoint_all = zeros(4,N,Npseudo+1);
...
for ii=1:N
    ...
    for jj=1:Neuler
        ...
        % check pseudosym vars
        for kk=1:Npseudo
            misorient_temp = misorientation(euler_indexed,eulerlist_pseudo(jj,:,kk));
            if misorient_temp<misorient
                igridcenter = jj;
                misorient = misorient_temp;
            end
    ...
    euler1 = eulerlist(igridcenter,:);    % grid center of original variant
        % doesn't matter if it's not the best var selected by DI, we'll check them all
    ...
    for kk=1:Npseudo
       % get grid center
        euler1 = eulerlist_pseudo(igridcenter,:,kk);    % grid center of pseudo variant
        q1 = eu2qu(euler1*pi/180);     % Quaternion of grid center

        % interpolate
        dp_grid = dp_all(Neuler*gridsize*kk+gridsize*(igridcenter-1)+1:Neuler*gridsize*kk+igridcenter*gridsize,ii);     % extract dp for pseudo variant grid
        dp_3d = reshapeto3d(dp_grid, 2*N_angles+1);  % reshape array to 3d
        [maxinterppoint,maxgridpoint,~,~,exitflag] = dpinterp(dp_3d, test_angles_1Drodrigues, q1);
        ...
        if exitflag>0
            if verbose==1
                fprintf('%g-v%g. Interpolation warning. Exitflag=%g.\n',ii,kk,exitflag);
            end
            warnings(end+1,:) = [ii kk exitflag];

            % store best grid point instead of interpolated point
            maxinterppoint_all(:,ii,kk+1) = maxgridpoint;
        else
            % store interpolated point
            maxinterppoint_all(:,ii,kk+1) = maxinterppoint;
        end 
    end
...
%%% figure out best variant
% initialize
eulerbest = zeros(N,3);     % euler angles of best variant (rad)
dpbest = zeros(N,1);        % dp of best variant
CI = zeros(N,1);            % dp diff btw best and 2nd best variants


% loop through each map point
for jj=1:N
    dp_temp = maxinterppoint_all(4,jj,:);    % get dp of pseudosym vars
    [dp_temp_max, imax] = max(dp_temp);     % figure out which var is best
    dp_temp(imax) = [];
    dp_temp_2nd = max(dp_temp);     % second highest dp

    % store
    eulerbest(jj,:) = maxinterppoint_all(1:3,jj,imax);
    dpbest(jj) = dp_temp_max;
    CI(jj) = dp_temp_max - dp_temp_2nd;
end
...
data.eulerlist_pseudo = eulerlist_pseudo;
